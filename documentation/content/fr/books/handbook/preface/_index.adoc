...
Le nouveau venu à FreeBSD constatera que la première section de ce livre, guide l'utilisateur à travers le processus d'installation de FreeBSD, présente progressivement les concepts et les conventions qui sont les fondements d'UNIX(R). Travailler avec cette section, demande un peu plus que le simple désire d'explorer et la capacité d'assimiler de nouveaux concepts quand ils sont présentés.

Une fois que vous en êtes arrivé là, la seconde section du Manuel bien plus grande, est une référence complète de tous les sujets qui intéressent les administrateurs systèmes de FreeBSD. Certains de ces chapitres peuvent vous recommander d'effectuer des lectures préliminaires, cela est noté dans le synopsis au début de chaque chapitre.
...
* Le chapitre crossref:config[config-tuning,Configuration et optimisation] a été mis à jour avec de nouvelles informations sur la gestion des ressources ACPI, de l'énergie sur l'utilitaire système `cron` et sur d'autres options supplémentaires d'optimisation du noyau.
* Le chapitre crossref:security[security,Sécurité] a été mis à jour avec de nouvelles informations sur les réseaux privés virtuels (VPNs), les listes de contrôle d'accès au système de fichiers (ACLs) et sur les avis de sécurité.
...
* Le chapitre crossref:disks[disks,Stockage des données] a bénéficié de l'ajout de nouvelles sections concernant les périphériques de stockage USB, les instantanés de systèmes de fichiers, les quotas d'utilisation des disques, les systèmes de fichiers réseaux et sauvegardés sur fichier et le chiffrage de partitions.
* crossref:vinum[vinum-vinum,Le gestionnaire de volume Vinum] est un nouveau chapitre apparaissant avec cette édition. Il décrit l'utilisation de Vinum, un gestionnaire de volume qui permet la création de disques logiques indépendants du périphérique et l'utilisation de systèmes RAID-0, RAID-1 et RAID-5 logiciels.
* Une section dépannage a été ajoutée au chapitre crossref:ppp-and-slip[ppp-and-slip,PPP et SLIP].
* Le chapitre crossref:mail[mail,Courrier électronique] bénéficie de nouvelles sections sur l'utilisation d'agents de transfert de courrier alternatifs, sur l'authentification SMTP, l'UUCP, fetchmail, procmail et d'autres sujets avancés.
* Le chapitre crossref:network-servers[network-servers,Serveurs réseau] apparaît avec cette édition. Ce chapitre traite de la configuration du serveur HTTP Apache, de ftpd, et celle d'un serveur pour clients Microsoft(R) Windows(R) à l'aide de Samba. Certaines sections du chapitre crossref:advanced-networking[advanced-networking,Administration réseau avancée], ont été déplacées vers ce nouveau chapitre.
* Le chapitre crossref:advanced-networking[advanced-networking,Administration réseau avancée] a été complété avec des informations sur l'utilisation des périphériques Bluetooth(R) sous FreeBSD, la configuration de réseaux sans fil et sur le mode de transfert réseau asynchrone (ATM).
...
* Le chapitre crossref:bsdinstall[bsdinstall,Installer FreeBSD] fut complètement réécrit avec de nombreuses captures d'écrans pour rendre le texte plus facile à comprendre pour les nouveaux utilisateurs.
* Le chapitre crossref:basics[basics,Quelques bases d'UNIX] a été mis à jour pour apporter des informations additionnelles sur les processus, les daemons et signaux.
* Le chapitre crossref:ports[ports,Installer des applications. les logiciels pré-compilés et les logiciels portés] a été mis à jour pour apporter des informations complémentaires sur la gestion des applications pré-compilées.
* Le chapitre crossref:x11[x11,Le système X Window] a complètement été réécrit en insistant sur l'utilisation de technologies modernes d'environnement de travail comme KDE et GNOME sous XFree86(TM) 4.X.
* Le chapitre crossref:boot[boot,Processus de démarrage de FreeBSD] a été mis à jour.
* Le chapitre crossref:disks[disks,Stockage des données] fut réécrit à partir de ce qui était à l'origine deux chapitres séparés "Disques" et "Sauvegardes". Nous pensons que le sujet est plus facile à appréhender quand il est présenté en un seul chapitre. Une section sur le RAID (matériel et logiciel) fut également ajoutée.
* Le chapitre crossref:serialcomms[serialcomms,Serial Communications] a été complètement réorganisé et mis à jour pour FreeBSD 4.X/5.X.
* Le chapitre crossref:ppp-and-slip[ppp-and-slip,PPP et SLIP] a été sensiblement mis à jour.
* Plusieurs nouvelles sections ont été ajouté au chapitre crossref:advanced-networking[advanced-networking,Administration réseau avancée].
* Le chapitre crossref:mail[mail,Courrier électronique] a été mis à jour pour inclure plus d'informations au sujet de la configuration de sendmail.
* Le chapitre crossref:linuxemu[linuxemu,Compatibilité binaire avec Linux®] a été mis à jour pour inclure des informations sur l'installation d'Oracle(R) et SAP(R) R/3(R).
...
Présente FreeBSD à un nouvel utilisateur. Il décrit l'histoire du projet FreeBSD, ses objectifs et son mode de développement.

_crossref:bsdinstall[bsdinstall,Installer FreeBSD]_::
Guide l'utilisateur à travers le processus d'installation. Quelques sujets d'installation avancée comme l'installation avec une console série, sont aussi couverts.
...
Couvre l'installation des logiciels tiers avec l'innovant "Catalogue de logiciels portés" de FreeBSD et des logiciels pré-compilés.
...
Liste les applications de bureautique courantes, comme les navigateurs Web, les suites de bureautique et décrit comment les installer sous FreeBSD.
...
Explique comment connecter des terminaux et des modems à votre système FreeBSD aussi bien pour les connexions entrantes que sortantes.
...
Explique la philosophie des pare-feux logiciels et fournit des informations détaillées sur la configuration des différents pare-feux disponibles pour FreeBSD.
